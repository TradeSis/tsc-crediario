...
def temp-table ttentrada no-undo serialize-name "aconegoc"
...
find aconegoc WHERE RECID(aconegoc) = ttentrada.id_recid
                      no-lock no-error.
...
do on error undo:
    find aconegoc WHERE RECID(aconegoc) = ttentrada.id_recid exclusive no-error.
