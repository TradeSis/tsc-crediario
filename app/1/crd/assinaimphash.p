...
if avail titulo
then 
    assign vprest = titulo.titvlcob /*- titulo.titdesc*/ /* Helio 31052024 a.        Valor da Prestacao: Não está considerando o valor do seguro e o mesmo deve sim ser somado, para bater com o valor da parcela do cupom impresso e do admcom */
...
varquivo = vdir + string(contrato.contnum) .

valor-divida = contrato.vlf_principal.
valor-total = contrato.vltotal - contrato.vlentra.
...
{crediario/app/1/crd/assinaimphash.i}
 ...
 run crediario/app/1/crd/assinapdf.p (
                  input varquivo,
                  input vdir, 
