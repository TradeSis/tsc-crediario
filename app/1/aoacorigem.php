...
if (isset($LOG_CAMINHO)) {
    ...
    $identificacao = date("dmYHis") . "-PID" . getmypid() . "-" . "aoacorigem";
    if (isset($LOG_NIVEL)) {
        if ($LOG_NIVEL >= 1) {
...
$retorno = $progr->executarprogress("crediario/app/1/aoacorigem", json_encode($jsonEntrada));
fwrite($arquivo, $identificacao . "-RETORNO->" . $retorno . "\n");
$dados = json_decode($retorno,true);
  if (isset($dados["conteudoSaida"][0])) { // Conteudo Saida - Caso de erro
      $dados = $dados["conteudoSaida"][0];
  } else {
      $dados = $dados["aoacorigem"];  
  }
